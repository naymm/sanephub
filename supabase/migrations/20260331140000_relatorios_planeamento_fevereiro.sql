-- Relatórios de planeamento para Fevereiro de 2026 (uma linha por empresa activa).
-- Mesma estrutura numérica que a seed do mês corrente, com textos ajustados a Fevereiro,
-- status Submetido e encargos pós-EBITDA coerentes com o EBITDA indicado.
-- Re-execução: ON CONFLICT (empresa_id, mes_ano) DO NOTHING.
...
select
  ...
  'Submetido',
  case e.codigo
    when 'SANEP-SGPS' then '["Fevereiro: coordenação do grupo e fecho de reportes mensais.","Reuniões de alinhamento orçamental do 1.º trimestre."]'
    when 'CREDIANGOLAR' then '["Concessão e reestruturação de crédito no arranque do ano.","Campanha de cobrança cooperativa pós-época festiva."]'
    when 'NOVA-FIBREX' then '["Fornecimento de fibras e materiais para projectos de rede em curso.","O&M e extensões acordadas para Fevereiro."]'
    when 'SANEP-LDA' then '["Prestação de serviços técnicos em Luanda — intensidade planificada em Fevereiro.","Acompanhamento de clientes PME após renovações de Janeiro."]'
    when 'SANEP-VIDA' then '["Comercialização de seguros de vida simbundos no período.","Parcerias com distribuidores e canal bancário em campanha de curto prazo."]'
    else '["Actividade comercial — Fevereiro 2026."]'
  end,
  case e.codigo
  ...
  case e.codigo
    when 'SANEP-SGPS' then 175000.00
    when 'CREDIANGOLAR' then 11000.00
    when 'NOVA-FIBREX' then 920000.00
    when 'SANEP-LDA' then 185000.00
    when 'SANEP-VIDA' then 38000.00
    else 0
  end,
  case e.codigo
    when 'SANEP-SGPS' then 115000.00
    when 'CREDIANGOLAR' then 6000.00
    when 'NOVA-FIBREX' then 580000.00
    when 'SANEP-LDA' then 155000.00
    when 'SANEP-VIDA' then 26000.00
    else 0
  end,
  case e.codigo
    when 'SANEP-SGPS' then 245000.00
    when 'CREDIANGOLAR' then 15000.00
    when 'NOVA-FIBREX' then 1820000.00
    when 'SANEP-LDA' then 410000.00
    when 'SANEP-VIDA' then 105000.00
    else 0
  end,
  case e.codigo
    when 'SANEP-SGPS' then 1850000.00
  ...
  case e.codigo
    when 'SANEP-SGPS' then '[{"banco":"BFA","numeroConta":"0012345001","saldoActual":8600000},{"banco":"BIC","numeroConta":"0098765002","saldoActual":3050000}]'::jsonb
    when 'CREDIANGOLAR' then '[{"banco":"BFA","numeroConta":"0020001003","saldoActual":1880000}]'::jsonb
    when 'NOVA-FIBREX' then '[{"banco":"BAI","numeroConta":"0035009007","saldoActual":12100000}]'::jsonb
    when 'SANEP-LDA' then '[{"banco":"BFA","numeroConta":"0041008005","saldoActual":4420000}]'::jsonb
    when 'SANEP-VIDA' then '[{"banco":"Standard Bank","numeroConta":"0052003001","saldoActual":2020000}]'::jsonb
    else '[]'::jsonb
  end,
  case e.codigo
    when 'SANEP-SGPS' then '[{"nome":"TechSupply Lda","valor":610000},{"nome":"Office Plus","valor":140000}]'::jsonb
    when 'CREDIANGOLAR' then '[{"nome":"Fornecedor equipamentos","valor":95000}]'::jsonb
    when 'NOVA-FIBREX' then '[{"nome":"Import Logistics SA","valor":4050000},{"nome":"Transportes Norte","valor":870000}]'::jsonb
    when 'SANEP-LDA' then '[{"nome":"Fuel & Services","valor":268000}]'::jsonb
    when 'SANEP-VIDA' then '[{"nome":"Agência publicidade","valor":150000}]'::jsonb
    else '[]'::jsonb
  end,
  case e.codigo
    when 'SANEP-SGPS' then '[{"nome":"Cliente Alpha","valor":2050000},{"nome":"Cliente Beta","valor":960000}]'::jsonb
    when 'CREDIANGOLAR' then '[{"nome":"Recuperações Fevereiro","valor":410000}]'::jsonb
    when 'NOVA-FIBREX' then '[{"nome":"Operador Nacional","valor":8750000}]'::jsonb
    when 'SANEP-LDA' then '[{"nome":"Contrato Ministério","valor":3400000}]'::jsonb
    when 'SANEP-VIDA' then '[{"nome":"Comissões corretores","valor":665000}]'::jsonb
    else '[]'::jsonb
  end,
  '2026-03-02 09:30:00+01'::timestamptz,
  'Planeamento — Fevereiro 2026'
